...
- **Curl/scripts and the qa-agent portal MUST send `x-profile`** or they get 401 — any new server-side self-call or worker callback must either forward it or send the service header `x-sheaf-service: analyzer-worker` (full middleware bypass; the worker's httpx client sends it on every call). Path exemptions: /healthz, POST /login, GET /users. Lesson (Jul 26 401 incident): path-list exemptions for the worker broke as soon as it made a call not on the list — exempt by service identity, not path.

...
- RULED Jul 26: approval = COPY (originals untouched); noted in feature docs that move/reference is a possible later efficiency. Q&A agent spec written: `internal_docs/_future/qa-agent.md` — agentic grep, no vectors; elements/pN.json projected to App Storage at ingest (prerequisite); citations = link chips that open the doc, never auto-open in chat.

...
## Page Review room (DIRECTION CHOSEN: A Filmstrip)
...
- Three interaction hypotheses live on canvas, Ops Desk skin held fixed: A Filmstrip (conveyor momentum, page is hero), B Queue+Inspector (inbox — command over the set), C Spotlight walkthrough (AI narrates one concern at a time; scope-chooser entry; veil + cutout). Frames homium-review-a/b/c + c-entry. DIRECTION CHOSEN (Jul 24, 2026, user): **A Filmstrip** — "by far the best" for the human-review page UI. Region/fraud-element highlighting liked as a FEATURE but explicitly v2 (= spec's md-offset→bbox highlight-on-page deferral). Two-layer armed-verdict pattern (born in C) still carries into every verdict surface.
...
- SHEAF BRAND (Jul 24, 2026, user decision): the product/app is **Sheaf** — Homium is the CLIENT (ratifies schemas, owns the program name in seed data), never in the app brand/titles/chrome. User's own Sheaf reference lives at artifacts/mockup-sandbox/public/sheaf-reference.html (top-bar mark: dark square + inner outline square + bold tight wordmark + "DOCUMENT OPS" micro-tag — adopted into the real AppShell in Ops Desk material).
- FRONT DOOR DOCTRINE (same round): user rejected landing on Form Templates ("impotent, no logo") — `/` is now a Dashboard (stat strip, blockers-first "Needs you" list, templates summary; all counts derived from API, never hardcoded); library lives at /templates; staff routes share AppShell, builder + intake form keep their own chrome.
- ANALYZER SOURCE OF TRUTH: Homium Analyzer spec v0.6.2 (attached_assets/homium-analyzer-spec-v0.6.2_*.md) supersedes ALL prior IDCM versions and is portal-aligned: pinned template = classification target (no separate checklist objects); write-path = POST /api/applications/{id}/analysis, one run object per call, portal store appends + owns latestRunId + validates suggestedBlockId. Accepted v1 portal items: optional docType on document blocks + builder dropdown (seeds tagged), pre-flight gate as first staged intake state, top-level projectedClosingDate, cost visible Originator+ (hidden from Applicant). Parser reality (verified Jul 2026): PaddleOCR-VL 1.6 only via self-hosted vLLM Docker on serverless GPU; third-party hosts only 0.9B-v1/1.5 (plug-compatible interim). LangExtract + vector index = v2; v1 fraud = metadata/visual/core-field consistency only (no amount checks — don't oversell). Verdicts human-only; triage UI must show document_date/expiry_date at accept (dates editable, edits recorded).

## Session decisions (Jul 24, 2026) — all locked with user
- **Test sign-in, never Clerk**: /login = 4 clickable seeded profiles (Ron Alvarez/Originator, Dana Whitfield/UW, Priya Nair/UW, Marcus Cole/Manager), localStorage-backed; verdicts record decidedBy from active profile. Real auth later, schema kept clean for it.
- **Email infra = v3.** "Request from applicant" = copy intake link until then.
- **Two-step choreography stays real even while analysis is simulated**: packet upload → deterministic pre-flight (no AI) → gate approval → only then a run lands. Simulated runs must POST through the real endpoint and be labeled simulated. (Stage 3.)
- **Parser: hosted PaddleOCR-VL 1.5 first**, self-hosted 1.6 in v2/v3; verify hosts at engine kickoff.
- **Judge = user's own API key** (multimodal; needs vision) — NOT Replit AI integration. Provider TBD (asked user).
- **App Storage approved** — replaces fixture:// URLs when engine starts. Python analyzer artifact beside Node portal OK'd.

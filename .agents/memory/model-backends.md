...
- Disarm RECURRED Jul 24 ~18:14Z, ~2h AFTER successful afternoon runs, with the account NOT suspended (control plane 200 OK) — treat disarms as recurring, not a one-off; check before any run session. Disarmed signature (read-only GET on the deployment): state READY but targetModelVersion + activeModelVersion both empty AND deployedModels?filter=deployment count 0 (healthy scale-to-zero keeps its deployedModel entry and blocks while spinning up; disarmed 404s instantly). Worker /health does NOT catch this (config-level checks only) — the failure surfaces at run time.

- Mistral key is FREE TIER: 4 requests/minute — unusable for multi-step agent loops (429 mid-turn). QA agent default is fireworks:glm-5p2 for this reason.

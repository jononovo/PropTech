...
import { Block, Template } from "@workspace/api-client-react";
import { requirementLabel, groupForPrimary, groupsSatisfiedBy, blockName, getSectionNumber, expiryLabel } from "./helpers";
...
export function DocDimensions({ 
  ...
  const req = block.requirement || "optional";
  if (req === "required_alt") reqIcon = <Link2 size={14} />;
  else if (req === "recommended") reqIcon = <CirclePlus size={14} />;
  else if (req === "optional") reqIcon = <CircleDashed size={14} />;
  
  items.push({ 
    id: "req", 
    icon: reqIcon, 
    label: requirementLabel(req), 
    important: req.startsWith("required") 
  });

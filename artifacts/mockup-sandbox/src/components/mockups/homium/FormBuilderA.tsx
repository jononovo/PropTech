...
import { PALETTE, PURCHASE_LOAN, templateToJson, templateStats, expiryLabel } from "./builderData";
    ...
    <>
      ...
      <div className="w-full h-[100dvh] flex flex-col font-sans bg-[#F3F5F7] text-[#0F172A] overflow-hidden relative selection:bg-[#BFDBFE] selection:text-[#1E40AF]">
        ...
        <header className="h-[52px] bg-white border-b border-[#E2E8F0] flex items-center px-4 shrink-0 z-10">
          ...
          <button onClick={toggleJson} className="font-mono text-[11px] font-medium text-[#64748B] hover:text-[#0F172A] mr-4 transition-colors">
            {`{ } JSON`}
          </button>
              ...
              <div className="space-y-2">
                {PALETTE.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-2 p-2.5 bg-white border border-[#E2E8F0] rounded-[4px] cursor-grab hover:border-[#94A3B8] transition-colors shadow-sm group">
                    <GripVertical size={14} className="text-[#94A3B8] mt-[2px] opacity-60 group-hover:opacity-100" />
                    <div>
          ...
          <main className="flex-1 overflow-auto p-8 pb-40">
            <div className="max-w-[760px] mx-auto">
              ...
              {PURCHASE_LOAN.sections.map((section, sIdx) => (
                <div key={section.id} ref={(el) => { sectionRefs.current[section.id] = el; }} className="mb-12 last:mb-0">
                  
                  {/* Section Header */}
                  <div className="flex items-center group mb-4">
                    <div className="font-mono text-[13px] text-[#94A3B8] w-8 font-medium">
                      {(sIdx + 1).toString().padStart(2, '0')}
                    </div>
                    <div className="text-[15px] font-semibold text-[#0F172A] flex-1">{section.name}</div>
                    <div className="px-1.5 py-0.5 bg-[#E2E8F0] text-[#475569] text-[9.5px] uppercase tracking-[0.06em] font-semibold rounded-[2px] mr-3">
                      {section.owner}
                    </div>
                    <GripVertical size={15} className="text-[#94A3B8] opacity-0 group-hover:opacity-100 cursor-grab transition-opacity" />
                  </div>
